    ...
    <div className="flex h-full flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold tracking-tight">Orbit Viewer & Space Situational Awareness</h1>
            <span className="rounded-full bg-cyan-500/10 px-2.5 py-0.5 text-[10px] font-mono font-medium text-cyan-400 border border-cyan-500/30">
              3D DIGITAL TWIN
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 rounded-full border border-border/80 bg-card/60 px-3 py-1 shadow-sm backdrop-blur-sm">
            ...
            <span className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground">
              {isWsConnected ? "WS Live Stream" : "SGP4 High-Precision Propagator"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-4">
        ...
        <div className="relative min-h-[440px] flex-1 overflow-hidden rounded-xl border border-border/80 shadow-2xl lg:col-span-3">
          {/* Top Mission Control Floating Toolbar */}
          <div className="absolute left-3 right-3 top-3 z-10 flex flex-wrap items-center justify-between gap-2 pointer-events-none">
            {/* View / Toggle Buttons */}
            <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-border/70 bg-card/80 p-1 shadow-lg backdrop-blur-md pointer-events-auto">
              {/* Auto Rotate */}
              <button
              ...
              <button
                ...
                title="Toggle 3D Orbit Trajectory Ribbons"
                className={cn(
                  "flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium transition-colors",
                  showAllOrbits
                    ? "bg-cyan-500/20 text-cyan-300 border border-cyan-500/40"
            ...
            <div className="flex items-center gap-1 rounded-lg border border-border/70 bg-card/80 p-1 shadow-lg backdrop-blur-md pointer-events-auto">
              <span className="px-2 text-[10px] font-mono uppercase text-muted-foreground flex items-center gap-1">
                <FastForward className="h-3 w-3" /> Warp:
              </span>
              {[1, 5, 15, 60].map((speed) => (
                <button
                  key={speed}
          ...
          {focusedSatellite && (
            <div className="absolute bottom-4 right-4 z-20 w-80 rounded-xl border border-cyan-500/40 bg-[#060B14]/90 p-4 shadow-2xl backdrop-blur-md text-foreground">
              <div className="flex items-start justify-between border-b border-border/60 pb-2">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="flex h-2 w-2 rounded-full bg-cyan-400 shadow-[0_0_8px_rgba(0,242,254,0.8)]" />
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="rounded-lg border border-border/40 bg-secondary/30 p-2">
                  ...
                  <p className="mt-0.5 text-base font-mono font-bold text-white">
                    {liveTelemetry?.altitudeKm ?? focusedSatellite.altitudeKm ?? "--"}{" "}
                ...
                <div className="col-span-2 rounded-lg border border-border/40 bg-secondary/30 p-2">
                  <div className="flex items-center justify-between text-muted-foreground text-[10px]">
                    ...
                    <span className="font-mono text-[9px] text-cyan-400">NADIR</span>
                  </div>
                  <div className="mt-1 flex items-center justify-between font-mono text-xs font-semibold text-white">
                    <span>
                      LAT:{" "}
                      {liveTelemetry
                        ? `${liveTelemetry.latitudeDeg > 0 ? "+" : ""}${liveTelemetry.latitudeDeg.toFixed(2)}°`
                        : `${focusedSatellite.latitudeDeg ?? 0}°`}
                    </span>
                    <span>
                      LON:{" "}
                      {liveTelemetry
                        ? `${liveTelemetry.longitudeDeg > 0 ? "+" : ""}${liveTelemetry.longitudeDeg.toFixed(2)}°`
                        : `${focusedSatellite.longitudeDeg ?? 0}°`}
                    </span>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-border/40 bg-secondary/30 p-2">
                  ...
                  <p className="font-mono font-semibold text-white">
                    {focusedSatellite.inclinationDeg ? `${focusedSatellite.inclinationDeg.toFixed(1)}°` : "--"}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-3 flex items-center justify-between border-t border-border/50 pt-2 text-[10px]">
                <span className="text-muted-foreground">Conjunction Status:</span>
          ...
          {/* Bottom Control Bar: Reset View & Quick Stats */}
          <div className="absolute bottom-3 left-3 z-10 flex items-center gap-2">
            <button
              ...
              title="Reset View"
              className="flex items-center gap-1.5 rounded-lg border border-border/70 bg-card/80 px-2.5 py-1.5 text-xs font-medium text-muted-foreground shadow-md backdrop-blur-md hover:bg-secondary hover:text-foreground transition-colors"
            >
