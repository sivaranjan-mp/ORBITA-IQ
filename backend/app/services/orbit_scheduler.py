...
async def update_orbit_states():
    ...
    async with async_session_maker() as db:
        try:
            ...
            result = await db.execute(select(Satellite).where(Satellite.status == 'active').options(selectinload(Satellite.orbit_state)))
...
def init_scheduler():
    scheduler.add_job(update_orbit_states, 'interval', minutes=5, id='update_orbit_states_job', replace_existing=True)
    scheduler.start()
