...
async def upload_cdm(request: CDMUploadRequest, current_user: UserProfile = Depends(get_current_user)):
    ...
    if not parsed.get("primary_norad_id") or not parsed.get("secondary_norad_id"):
        raise HTTPException(status_code=400, detail="Missing primary or secondary NORAD IDs in CDM")

    if not parsed.get("tca"):
        raise HTTPException(status_code=400, detail="Missing TCA (Time of Closest Approach)")
    ...
    if not cdm_res.data:
        raise HTTPException(status_code=500, detail="Failed to insert CDM record")
