...
async def upload_omm(request: OMMUploadRequest, current_user: UserProfile = Depends(get_current_user)):
    ...
    if current_user.role not in ["admin", "operator"]:
        raise HTTPException(status_code=403, detail="Not authorized to upload telemetry")
    ...
    if not norad_id:
        raise HTTPException(status_code=400, detail="Missing NORAD_CAT_ID in OMM payload")
        
    # Check if satellite exists
    sat_res = admin.table("satellites").select("id").eq("norad_id", norad_id).maybe_single().execute()
    if not sat_res or not sat_res.data:
        raise HTTPException(status_code=404, detail=f"Satellite with NORAD ID {norad_id} not found in database")
    ...
    if not insert_res.data:
        raise HTTPException(status_code=500, detail="Failed to insert OMM record")
